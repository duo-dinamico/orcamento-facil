...
class AddIncomePopUp(tk.Toplevel):
    def __init__(self, parent, presenter) -> None:
        ...
        entry_income_name = ttk.Entry(self, textvariable=self.income_name)
        ...
        self.combobox_target_account = ttk.Combobox(self, state="readonly", values=target_accounts)
        self.combobox_target_account.current(0)
        self.combobox_target_account.pack(anchor="w", padx=10, pady=5, fill="x")

        label_expected_income = ttk.Label(self, text="Expected income")
        label_expected_income.pack(anchor="w", padx=10, pady=5, fill="x")
        entry_expected_income = ttk.Entry(self, textvariable=self.expected_income)
        ...
        entry_real_income = ttk.Entry(self, textvariable=self.real_income)
        ...
        entry_income_day = ttk.Entry(frame_income_date, textvariable=self.income_day)
        ...
        self.combobox_income_month = ttk.Combobox(frame_income_date, state="readonly", values=income_month)
        self.combobox_income_month.pack(side="left", expand=True, fill="both")

        label_recurrence = ttk.Label(self, text="Recurrence")
        label_recurrence.pack(anchor="w", padx=10, pady=5, fill="x")
        self.combobox_recurrence = ttk.Combobox(self, state="readonly", values=recurrence)
        self.combobox_recurrence.pack(anchor="w", padx=10, pady=5, fill="x")
        ...
        button_add_income = ttk.Button(self, text="Add income source")
        button_add_income.pack(side="right", padx=10, pady=5)
        button_add_income.bind("<Button-1>", presenter.handle_add_income)
