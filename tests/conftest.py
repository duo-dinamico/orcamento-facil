...
def fixture_valid_user(db_session):
    db_session.create_user(username="validUser", password=get_hashed_password("ValidPassword1"))


@pytest.fixture(name="valid_account")
def fixture_valid_account(db_session, valid_user):
    _ = valid_user
    db_session.create_account(user_id=1, name="validAccount")


@pytest.fixture(name="valid_category")
def fixture_valid_category(db_session):
    db_session.create_category(name="validCategory")


@pytest.fixture()
def second_valid_category(db_session):
    db_session.create_category(name="secondvalidCategory")


@pytest.fixture(name="valid_subcategory")
def fixture_valid_subcategory(db_session, valid_category):
    _ = valid_category
    db_session.create_subcategory(category_id=1, name="validSubCategory")
...
def valid_income(db_session, valid_account, valid_user):
    _ = valid_account
    _ = valid_user
    db_session.create_income(account_id=1, user_id=1, name="validIncome")
...
def valid_income_second(db_session, valid_account, valid_user):
    _ = valid_account
    _ = valid_user
    db_session.create_income(account_id=1, user_id=1, name="validIncomeSecond")
